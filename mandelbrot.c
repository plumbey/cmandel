...
inline double
pointIterate(double x0, double y0, int max)
{
	double x = 0, y = 0;
	double iterations = 0;
...
void
generateMandelbrot(gdImagePtr img, int colorIn)
{
	const double xLower = xCenter - delta;
	const double xUpper = xCenter + delta;

	const double yLower = -yCenter - delta * ((double)HEIGHT / WIDTH);
	const double yUpper = -yCenter + delta * ((double)HEIGHT / WIDTH);
	...
	for (int i = 0; i < WIDTH; i++) {
		...
		for (int j = 0; j < HEIGHT; j++) {
			double y0 = yLower + yDifference * j / HEIGHT;
			...
			if (colorIn && iteration == max) {
				...
				color.h = (int)(powf((iteration / max) * 360,
						     hueIntensity)) %
					  360;
				color.s = 1;
