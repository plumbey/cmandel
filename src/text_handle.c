...
printHelp()
{
	...
	printf(
	    "specifies the distance between coordinate center points and their "
	    "bounds\n");
	...
	printf(
	    "this is evaluated as an exponent so be careful with it (or don't I "
	    "guess)\n");
	...
	printf(
	    "true or false specifier setting whether the set should be colored "
	    "black\n");
...
int
parseArgs(int argc, char *argv[], MandelData *data)
{
	data->width = 3000;
	data->height = 3000;
	data->delta = 1.0;
	...
	for (int i = 0; i < argc; i++) {
		if (strcmp(argv[i], "-w") == 0 && i + 1 < argc) {
			...
			if (m == 0) {
				printf(
				    "Error! Invalid max iteration specified\n");
				exit(1);
			}
			data->iterMax = m;
			...
			// Add ./ if no slash
			const char *prefix = has_slash ? "" : "./";
			const char *suffix = has_png ? "" : ".png";
			size_t needed =
			    strlen(prefix) + strlen(name) + strlen(suffix) + 1;
			char *temp = malloc(needed);
			snprintf(temp, needed, "%s%s%s", prefix, name, suffix);
			data->output = temp;
...
int
printMandelDataToStream(const MandelData *data, FILE *stream)
{
	if (!stream)
		return 0;
...
int
appendMandelDataToFile(const MandelData *data, const char *path)
{
