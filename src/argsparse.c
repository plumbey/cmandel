...
void print_help() {
  ...
  printf(
      "specifies as an integer how many threads will be allocated to cMandel. "
...
void parse_args(int argc, char *argv[], MandelData *data) {
  ...
  for (int i = 0; i < argc; i++) {
    if (strcmp(argv[i], "-w") == 0 && i + 1 < argc) {
      ...
      // Add ./ if no slash
      char temp[1024] = {0};
      if (!has_slash) strcat(temp, "./");

      strcat(temp, name);

      // Add .png if missing
      if (!has_png) strcat(temp, ".png");
      ...
      if ((t == 0) || (t > omp_get_max_threads())) {
        printf("Error! Invalid thread count specified. Max threads: %i\n",
               omp_get_max_threads());
        exit(1);
      }
      data->numThreads = t;
