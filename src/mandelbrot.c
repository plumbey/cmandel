...
void
generateMandelbrot(gdImagePtr img, const MandelData *data)
{
	...
	const double yLower =
	    -(data->yCenter) -
	    data->delta * ((double)data->height / data->width);
	const double yUpper =
	    -(data->yCenter) +
	    data->delta * ((double)data->height / data->width);
	...
	for (int i = 0; i < data->width; i++) {
		...
		for (int j = 0; j < data->height; j++) {
			...
			double iteration = pointIteratePeriodic(
			    x0, y0, data->iterMax, data->delta);
			hsv color;
			...
			if (data->colorIn && iteration == data->iterMax) {
				...
				color.h = (int)(powf(
					      (iteration / data->iterMax) * 360,
					      data->huePower)) %
					  360;
				color.s = 1;
				color.v = powf(iteration / data->iterMax,
					       data->darkness);
			}

			// int toAdd = allocHexToImage(img, hsvToRgb(color));
			int toAdd =
			    allocHexToImage(img, hsvToPal(color, data->colors));
